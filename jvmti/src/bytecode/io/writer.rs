...
impl<'a> ClassWriter<'a> {
    ...
    fn write_stack_map_table(&mut self, table: &[StackMapFrame], cp: &ConstantPool) -> Result<usize, Error> {
    ...
    fn write_instructions(&mut self, instructions: &[Instruction]) -> Result<usize, Error> {
        let mut target: Vec<u8> = vec![];
        let _ /*written_bytes*/ = {
    ...
    fn write_exception_handlers(&mut self, exception_table: &[ExceptionHandler]) -> Result<usize, Error> {
    ...
    pub fn write_n(&mut self, bytes: &[u8]) -> Result<usize, Error> {
