...
impl ClassfilePrinter {
    ...
    pub fn render_constant(constant: &Constant, pool: &ConstantPool) -> String {
        match constant {
            ...
            Constant::Placeholder => "Placeholder".into()
        }
    }
    ...
    pub fn render_attribute(code: &Attribute, cp: &ConstantPool) -> Vec<String> {
        ...
        match code {
            Attribute::Code { max_stack: ref ms, max_locals: ref ml, code: ref c, exception_table: ref _et, ref attributes } => {
                ...
                let _: Vec<()> = c.iter().map(|instr| (instr.len(), match instr {
                    ...
                    Instruction::IF_ACMPEQ(_value) => format!("if_acmpeq"),
                    Instruction::IF_ACMPNE(_value) => format!("if_acmpne"),
                    Instruction::IF_ICMPEQ(_value) => format!("if_icmpeq"),
                    Instruction::IF_ICMPNE(_value) => format!("if_icmpne"),
                    Instruction::IF_ICMPLT(_value) => format!("if_icmplt"),
                    Instruction::IF_ICMPGE(_value) => format!("if_icmpge"),
                    Instruction::IF_ICMPGT(_value) => format!("if_icmpgt"),
                    Instruction::IF_ICMPLE(_value) => format!("if_icmple"),
                    Instruction::IFEQ(_value) => format!("ifeq"),
                    Instruction::IFNE(_value) => format!("ifne"),
                    Instruction::IFLT(_value) => format!("iflt"),
                    Instruction::IFGE(_value) => format!("ifge"),
                    Instruction::IFGT(_value) => format!("ifgt"),
                    Instruction::IFLE(_value) => format!("ifle"),
                    Instruction::IFNONNULL(_value) => format!("ifnonnull"),
                    Instruction::IFNULL(_value) => format!("ifnull"),
                    Instruction::IINC(_value, _increment) => format!("iinc"),
                    Instruction::ILOAD(_value) => format!("iload"),
                    ...
                    Instruction::INSTANCEOF(_value) => format!("instanceof"),
                    Instruction::INVOKEDYNAMIC(value) => format!("invokedynamic #{}", value),
                    Instruction::INVOKEINTERFACE(value, _index) => format!("invokeinterface #{}", value),
                    ...
                    Instruction::JSR(_value) => format!("jsr"),
                    Instruction::JSR_W(_value) => format!("jsr_w"),
                    ...
                    Instruction::LDC(_value) => format!("ldc"),
                    Instruction::LDC_W(_value) => format!("ldc_w"),
                    Instruction::LDC2_W(_value) => format!("ldc2_w"),
                    Instruction::LDIV => format!("ldiv"),
                    Instruction::LLOAD(_value) => format!("lload"),
                    ...
                    Instruction::LOOKUPSWITCH(_value, ref _table) => format!("lookupswitch"),
                    ...
                    Instruction::MULTIANEWARRAY(_value, _size) => format!("multianewarray"),
                    Instruction::NEW(_value) => format!("new"),
                    Instruction::NEWARRAY(_value) => format!("newarray"),
                    Instruction::NOP => format!("nop"),
                    Instruction::POP => format!("pop"),
                    Instruction::POP2 => format!("pop2"),
                    Instruction::PUTFIELD(_value) => format!("putfield"),
                    Instruction::PUTSTATIC(_value) => format!("putstatic"),
                    Instruction::RET(_value) => format!("ret"),
                    ...
                    Instruction::TABLESWITCH(_value, _, _, _) => format!("tableswitch"),
                    Instruction::IINC_W(_value, _increment) => format!("iinc_w"),
    ...
    pub fn render_stack_map_frame(frame: &StackMapFrame) -> String {
        match frame {
            StackMapFrame::SameFrame { tag } => format!("SameFrame {}", tag),
            StackMapFrame::SameLocals1StackItemFrame { tag, stack: _ /*VerificationType*/ } => format!("SameLocals1StackItemFrame {}", tag),
            ...
            StackMapFrame::ChopFrame { tag, offset_delta: offset } => format!("ChopFrame {} {}", tag, offset),
            StackMapFrame::SameFrameExtended { offset_delta: offset } => format!("SameFrameExtended {}", offset),
            StackMapFrame::AppendFrame { tag, offset_delta: offset, locals: _ /*Vec<VerificationType>*/ } => format!("AppendFrame {} {}", tag, offset),
            ...
            StackMapFrame::FutureUse { tag: _tag } => "FutureUse".into()
        }
    }
    ...
    pub fn render_line_number_table(table: &[LineNumberTable]) -> Vec<String> {
